...
MIN_DTE = 7           # 最小到期天數
MAX_DTE = 60           # 最大到期天數
STRIKE_PCT_MIN = 0.80  # 履約價下限 (現價 * 0.80)
STRIKE_PCT_MAX = 0.95  # 履約價上限 (現價 * 0.95)
MIN_ANNUAL_RETURN = 0.05 # 最小年化報酬率過濾 (15%)
MIN_SHARPE_RATIO = 0.2   # 最小夏普值過濾 (0.3)
...
def get_current_price(ib: IB, contract: Stock) -> float:
    ...
    try:
        ...
        print(f"DEBUG: Requesting Moomoo quote for {moomoo_symbol}...")
        quote_ctx = OpenQuoteContext(host=moomoo_host, port=moomoo_port)
        
        # Subscribe first to ensure data availability
        from moomoo import SubType
        quote_ctx.subscribe([moomoo_symbol], [SubType.QUOTE], subscribe_push=False)
        ...
        if ret == RET_OK:
            if not df.empty:
                print(f"DEBUG: Moomoo quote result:\n{df[['code', 'last_price', 'close_price']]}")
                row = df.iloc[0]
                ...
                print("DEBUG: Moomoo returned empty dataframe.")
        else:
            print(f"DEBUG: Moomoo get_stock_quote failed. ret={ret}")
...
def get_moomoo_positions(ib_symbol: str, underlying_price: float, hv_1w: float, hv_6m: float):
    ...
    try:
        ...
        if filtered.empty:
            ...
            # 這裡我們放寬前面的匹配，並且 strike 可能為 5-9 位數 (PYPL 可能是 5 位，如 57000)
...
def main():
    ...
    try:
        ...
        for res in ranked_results[:top_n]:
            ...
            # 在顯示時，針對 Sharpe 高的給予顏色
            sharpe_str = f"{res['sharpe']:<4.2f}"
            if res['sharpe'] > 1.0:
                sharpe_str = f"{COLOR_GREEN}{sharpe_str}{COLOR_RESET}"
            elif res['sharpe'] > 0.5:
                 sharpe_str = f"{COLOR_BLUE}{sharpe_str}{COLOR_RESET}"
            ...
            # 組合物件
            row_str = f"{res['expiry']:<10} | {res['dte']:<4} | {res['strike']:<8.1f} | {price_field} | {res['return']*100:<6.1f} | {res['annualized']*100:<6.1f} | {res['ideal_bid']:<8.2f} | {ratio:<6.2f} | {sharpe_str:<4} | {iv_str:<6} | {hv_str:<6} | {delta_str:>7} | {res['kelly']*100:>6.1f}%"

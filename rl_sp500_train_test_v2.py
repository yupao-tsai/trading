...
try:
    ...
    raise RuntimeError("Missing torch. Install: pip install torch") from e
...
def detect_best_device() -> str:
    """
    Prefer CUDA GPU if available, else Apple MPS if available, else CPU.
    Returned string is suitable for SB3 'device' argument.
    """
    # CUDA (NVIDIA)
    try:
...
def resolve_device_arg(device_arg: str) -> str:
    """
    device_arg in {auto,cpu,cuda,mps}
    - auto -> detect_best_device()
    ...
    if d in ("cpu", "cuda", "mps"):
        # validate availability lightly
        if d == "cuda" and not torch.cuda.is_available():
            _warn("[DEVICE] --device cuda specified but torch.cuda.is_available() is False. Falling back to cpu.")
            return "cpu"
        if d == "mps":
            ok = False
            try:
                ...
                ok = False
            if not ok:
                _warn("[DEVICE] --device mps specified but MPS is not available. Falling back to cpu.")
                return "cpu"
        return d

    _warn(f"[DEVICE] Unknown --device '{device_arg}', fallback to auto-detect.")
    return detect_best_device()
...
class PortfolioEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self._w = np.zeros((self.N,), dtype=np.float32)
...
def run_train(
    ...
    # Resolve device
    resolved_device = resolve_device_arg(args.device)
    _info(f"[DEVICE] requested={args.device} resolved={resolved_device}")
    ...
    # Fix SB3 warning: use dict instead of list wrapper
    policy_kwargs = dict(
...
def run_test(
    ...
    # Resolve device (for loading model onto the same device)
    resolved_device = resolve_device_arg(args.device)
    _info(f"[DEVICE] requested={args.device} resolved={resolved_device}")
    ...
    model = PPO.load(model_path, device=resolved_device)
...
def main():
    ...
    # Show torch device availability
    try:
        _info(f"[TORCH] cuda_available={torch.cuda.is_available()}")
    except Exception:
        _info("[TORCH] cuda_available=unknown")

    try:
        mps_ok = hasattr(torch.backends, "mps") and torch.backends.mps.is_available()
        _info(f"[TORCH] mps_available={mps_ok}")
    except Exception:
        _info("[TORCH] mps_available=unknown")

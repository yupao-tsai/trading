    ...
    def update_leg_status(
        ...
        with self._lock:
            ...
            def _is_filled(x: LegFill) -> bool:
                return normalize_status(x.status) == "FILLED"
            ...
            def _is_cancel(x: LegFill) -> bool:
                ...
                return s == "CANCELLED" or s == "UserCanceled" or s == "SystemCanceled"
...
class ExecutionEngine:
    ...
    def on_signal(self, sig: Dict[str, Any], api: sj.Shioaji):
        ...
        self.pos_mgr.prepare_phase(
            stock_code=stock_code,
            phase=phase,
            target_stock_shares=STOCK_QTY, # ✅ Use Lots (張)
        ...
        if is_open:
            if filled_f and (not filled_s):
                ...
                if est_net >= MIN_PROFIT_TO_REPAIR:
                    ...
                    try:
                        ...
                        self.tracker.record(oid, stock_code, future_code, phase, "stock", sj.constant.Action.Buy, hedge_stock_buy, STOCK_QTY)
                ...
                try:
                    trade_fix = self._place_future_order(api, future_code, sj.constant.Action.Buy, unwind_fut_buy, FUTURE_QTY, phase_open=False)
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "future", sj.constant.Action.Buy, unwind_fut_buy, FUTURE_QTY)
            ...
            if filled_s and (not filled_f):
                ...
                if est_net >= MIN_PROFIT_TO_REPAIR:
                    ...
                    try:
                        ...
                        oid = str(trade_fix.order.id)
                        self.tracker.record(oid, stock_code, future_code, phase, "future", sj.constant.Action.Sell, hedge_fut_sell, FUTURE_QTY)
                        self.pos_mgr.register_order(oid, stock_code, phase, "future")
                    except Exception as e:
                ...
                try:
                    trade_fix = self._place_stock_order(api, stock_code, sj.constant.Action.Sell, unwind_stock_sell, STOCK_QTY)
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "stock", sj.constant.Action.Sell, unwind_stock_sell, STOCK_QTY)
                    self.pos_mgr.register_order(oid, stock_code, phase, "stock")
                except Exception as e:
            ...
            if filled_f and (not filled_s):
                ...
                try:
                    ...
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "stock", sj.constant.Action.Sell, hedge_stock_sell, STOCK_QTY)
                    self.pos_mgr.register_order(oid, stock_code, phase, "stock")
                except Exception as e:
                ...
                try:
                    trade_fix = self._place_future_order(api, future_code, sj.constant.Action.Sell, unwind_fut_sell, FUTURE_QTY, phase_open=True)
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "future", sj.constant.Action.Sell, unwind_fut_sell, FUTURE_QTY)
                    self.pos_mgr.register_order(oid, stock_code, phase, "future")
                except Exception as e:
            ...
            if filled_s and (not filled_f):
                ...
                try:
                    ...
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "future", sj.constant.Action.Buy, hedge_fut_buy, FUTURE_QTY)
                    self.pos_mgr.register_order(oid, stock_code, phase, "future")
                except Exception as e:
                ...
                try:
                    trade_fix = self._place_stock_order(api, stock_code, sj.constant.Action.Buy, unwind_stock_buy, STOCK_QTY)
                    oid = str(trade_fix.order.id)
                    self.tracker.record(oid, stock_code, future_code, phase, "stock", sj.constant.Action.Buy, unwind_stock_buy, STOCK_QTY)
                    self.pos_mgr.register_order(oid, stock_code, phase, "stock")
                except Exception as e:
...
class ArbitrageSystem:
    ...
    def _on_deal_event(self, trade_dict):
        ...
        # print(f"[Deal Event] trade={trade_dict}")
        self.pos_mgr.record_order_event(None, trade_dict)

        order_id = str(trade_dict.get("trade_id") or (trade_dict.get("order") or {}).get("id") or "")
        if not order_id:
            return
        ...
        if isinstance(st_node, dict):
            qty = int(st_node.get("deal_quantity") or 0)
        ...
        # 判斷狀態
        # 這裡很難知道 Order 的總量 (除非去查 PosMgr 的 target)
        # 簡單起見，有成交就叫 PARTFILLED，如果 PosMgr 發現滿了會自己改成 FILLED (需修改 PosMgr)
        # 或者這裡先給 FILLED，PosMgr 邏輯是：如果是 FILLED，代表這筆 deal 讓狀態變 Filled？
        # 不，狀態應該反映「整張單」的狀態。
        # 如果我們無法知道整張單是否已滿，暫時標記為 PARTFILLED，除非 API 明確說 Filled。
        # Shioaji Deal Event 沒有 status 欄位說 "Filled"，它只是成交回報。
        # 為了配合 sync.done，我們需要知道何時全成。
        # 妥協：這裡更新量，狀態設為 PARTFILLED。
        # PosMgr 的 update_leg_status 會累加量。我們需要在 PosMgr 裡判斷：如果 累加量 >= 目標量，則轉 FILLED。
        # 目前 PosMgr 比較笨，依賴傳入的 status。
        # 我們讓 status = "PARTFILLED" (有量)，然後讓 PosMgr 決定。
        # 為了讓 sync.done 觸發，如果這筆成交量很大（假設全成），我們可能需要更聰明。
        # 但通常 Deal Event 會伴隨 Order Event (Status=Filled)。
        # 所以：Deal Event 負責加量，Order Event 負責轉狀態為 Filled。這樣最完美。
        
        print(f"[Deal Parsed] {stock_code} {leg} qty={qty} px={px}")

        self.pos_mgr.update_leg_status(
            stock_code=stock_code,
            phase=phase,
            leg=leg,
            status="PARTFILLED", # 暫定，靠 Order Event 轉 FILLED，或 PosMgr 累加判斷
            filled_qty=qty,
            deal_price=px,
    ...
    def start(self, top_n_pairs: int = TOP_N_PAIRS, refresh_interval_sec: int = REFRESH_PAIRS_EVERY_SEC):
        ...
        self.account_monitor = AccountMonitor(
            ...
            poll_sec=15.0, # ✅ 調高至 15s 避免 API Limit
        )

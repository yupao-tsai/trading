...
HOST = "127.0.0.1"
PORT = 7496
CLIENT_ID = 13 # Changed ID to avoid conflict
...
def qualify_stock(ib: IB, symbol: str) -> Stock:
    # Handle BRK B case
    if symbol == "BRK.B" or symbol == "BRKB":
        symbol = "BRK B"
    
    stk = Stock(symbol, "SMART", currency=CURRENCY) # Removed primaryExchange=PRIMARY_EXCHANGE to be more flexible, or keep it if needed
    if symbol == "BRK B":
        stk.primaryExchange = "NYSE" # BRK B often needs specific exchange or SMART works better without it? Original code used NYSE.
...
def get_iv_rank_data(ib: IB, contract: Stock) -> Tuple[float, float, float]:
    """
    獲取過去一年的 IV High, IV Low, IV Current (from history)
    Returns: (iv_low, iv_high, iv_current_hist)
    """
...
def get_next_earnings_date(ib: IB, contract: Stock) -> Optional[datetime.date]:
    ...
    try:
        # CalendarReport requires 'Fundamental' subscription
        ...
        # Structure: <EarningsList><Earnings><Date>...</Date></Earnings>...</EarningsList>
        ...
        for earnings in root.findall(".//Earnings"):
            ...
            if date_elem is not None and date_elem.text:
                e_date_str = date_elem.text # Usually YYYY-MM-DD
                try:
        ...
        # print(f"Failed to fetch earnings date: {e}")
        return None
...
def get_unified_option_price(ib: IB, symbol: str, expiry_str: str, strike: float, right: str, moomoo_prices: Dict = None) -> Tuple[float, str, float, float]:
    ...
    try:
        ...
        if ticker.modelGreeks:
            ...
            if ticker.modelGreeks.theta: theta = ticker.modelGreeks.theta # Added Theta capture
...
def analyze_candidates(ib: IB, candidates: List[Option], current_price: float, hv_1w: float, hv_6m: float, iv_rank_info: Tuple[float, float, float] = (0,0,0), earnings_date: Optional[datetime.date] = None, symbol_for_moomoo: str = "") -> List[Dict]:
    """對候選合約請求報價並計算指標"""
    results = []
    ...
    moomoo_prices = get_moomoo_quote_for_options(symbol_for_moomoo, MIN_DTE, MAX_DTE)
    ...
    for i in range(0, len(candidates), chunk_size):
        ...
        for opt, ticker in tickers:
            ...
            if final_price > 0:
                ...
                cache_key = (symbol_for_moomoo, expiry_display, opt.strike, opt.right)
            ...
            results.append({
                ...
                "conId": opt.conId,
                "hv": max(hv_1w, hv_6m) # Add HV for record
            })
...
def main():
    ...
    try:
        for symbol in ALL_TARGETS:
            ...
            if price <= 0:
                print(f"❌ Cannot retrieve underlying price for {symbol}. Skipping.")
                continue

            # 1.5 取得歷史波動率
            hv_1w, hv_6m = get_historical_volatility(ib, stk)
            
            # 1.6 Fetch IV Rank Data & Earnings
            ...
            # Pass symbol string for Moomoo cache lookup
            ranked_results = analyze_candidates(ib, candidates, price, hv_1w, hv_6m, (iv_low, iv_high, iv_curr_hist), earnings_date, symbol_for_moomoo=symbol)

            # 5. Append results to file immediately (Streaming style)
            if ranked_results:
                ...
                print(f"  Appended {len(ranked_results)} opportunities to candidates.txt")

                # --- Print to Console (Match main_union.py style) ---
                ...
                for res in ranked_results[:top_n]:
                    ...
                    def color_cell(text, condition):
                        if is_perfect: return text 
                        if condition: return f"{COLOR_GREEN}{text}{COLOR_RESET}"
                        return text
                    ...
                    if is_perfect:
                        ...
                        if res['earn']: row_str += f" {COLOR_RED}(EARN){COLOR_RESET}"
                        print(row_str)

...
def analyze_candidates(
    ...
    for i in range(0, len(candidates), chunk_size):
        ...
        for opt, ticker in tickers:
            ...
            delta = 0.0
            theta = 0.0
            gamma = 0.0 # [MODIFIED] Added Gamma
            ...
            if ticker.modelGreeks:
                ...
                if ticker.modelGreeks.gamma: # [MODIFIED] Fetch Gamma
            ...
            if gamma == 0:
                gamma = bs_gamma(current_price, opt.strike, T, RISK_FREE_RATE, iv)
            ...
            # [MODIFIED] Calculate T/G Ratio
            # Gamma is usually small, so T/G can be large. 
            # Theta is "gain per day" (negative for buyer, positive for seller logic in text, though mathematical theta is negative)
            # Gamma is acceleration.
            # We want High Theta, Low Gamma. So abs(Theta) / Gamma.
            tg_ratio = abs(theta) / abs(gamma) if abs(gamma) > 0.00001 else 0.0
            ...
            results.append({
                ...
                "iv": iv,
                "delta": delta,
                "gamma": gamma,    # [MODIFIED]
                "sharpe": sharpe,
                "ivr": ivr,
                "safety": safety,
                "td_ratio": td_ratio,
                "theta_yield": theta_yield,
                "tg_ratio": tg_ratio, # [MODIFIED]
                "is_iv_est": is_iv_est,
...
def format_candidate_line(r: Dict, hv_val: float) -> str:
    # TSV columns (updated)
    # [MODIFIED] Added Gamma and T/G columns at the end
    fees_per_share = FEES_PER_CONTRACT / 100.0
...
def is_perfect_candidate(r: Dict, hv_val: float) -> bool:
    ...
    # [MODIFIED] Optional: can add T/G check here, e.g. tg_ratio > 0.5
    # For now, keep original logic + DTE filter handles the main safety issue.
...
def scan_one_symbol(ib: IB, symbol: str, top_n: int) -> Tuple[List[Dict], float, Optional[datetime.date]]:
    ...
    print("Thresholds: Ann>=15%, IVR>30, T/D>0.4, |Delta|<0.30, IV>HV, DTE>30")
    ...
    print("New Metrics: Gamma (Acceleration), T/G (Theta/Gamma Ratio - Stability)")

    # [MODIFIED] Added Gamma and T/G
    header = (
        f"{'Expiry':<10} | {'DTE':<4} | {'Strike':<8} | {'Bid(Src)':<16} | "
        f"{'Ann%':<6} | {'Ideal':<8} | {'Ratio':<6} | {'IVR':<5} | {'IV':<7} | {'Delta':<8} | "
        f"{'Gamma':<8} | {'Shp':<5} | {'T/D':<5} | {'T/G':<6} | {'Kelly%':<7}"
    )
    # Adjusted layout slightly to fit
    print(header)
    print("-" * len(header))
    ...
    for r in ranked[:top_n]:
        ...
        row = (
            ...
            f"{ann_str:<6} | {ideal_str:<8} | {ratio_str:<6} | {r['ivr']:<5.0f} | {iv_str:<7} | {delta_str:<8} | "
            f"{gamma_str:<8} | {r['sharpe']:<5.2f} | {td_str:<5} | {tg_str:<6} | {kelly_str:<7}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Union Sell Put Scanner (v8) - Enhanced logic (30-60DTE, Gamma, T/G)")
    ...
    parser.add_argument("--out-file", default="candidate_v8.txt", help="Output candidate file (default: candidate_v8.txt)")
    ...
    TSV_HEADER = "\t".join([
        ...
        "IV", "HV", "Theta", "Delta", "Gamma", "Kelly%", "IVR", "T/D", "T/G", "OI"
    ])
    if not args.append:
    ...
    print(f"[Filter] DTE={MIN_DTE}-{MAX_DTE}, Open Interest>={MIN_OPEN_INTEREST}")
    print(f"[Logic] Defensive: DTE>=30, T/G Displayed. Offensive: Sort by Kelly.")

...
class PairDiscoverer:
    ...
    def find_active_pairs(self, top_n: int = TOP_N_PAIRS) -> List[Tuple[str, str]]:
        ...
        for contract in candidates:
            ...
            # --- Filter: Price Cap (New) ---
            # 透過 Daily Quotes Cache 快速檢查收盤價，若 > 500 則跳過
            # 如果 Daily Quotes 沒資料，暫時不擋 (或保守起見擋掉)
            # 這裡假設 daily_quotes_map 存的是 Volume，我們需要擴充它或另外查價格
            # 為了簡單，我們這裡先用 snapshot 檢查價格 (會稍微慢一點，但精確)
            # 或者，直接用 contract.reference (如果有的話)
            
            # 實作：利用 snapshot 檢查現貨價格
            # 為了效能，我們只對「成交量合格」的標的查價格，避免 API call 爆炸
            # 所以把價格檢查放在 Volume Check 之後
            ...
            # --- Check Price < 500 ---
            # 這裡我們需要一個 helper 快速拿價格
            # 優先用 Daily Quotes (如果我們有存價格的話) -> 目前只存 Volume
            # 所以這裡用 Snapshot 抽查
            try:
                stk_contract = self.api.Contracts.Stocks[stock_code]
                # 這裡做一個簡單的優化：如果沒 snapshot 就假設它合格，或者嚴格一點
                # 考慮到效能，我們可以只對前 N 名做這個檢查，或者忍受一點延遲
                # 這裡採用: 嘗試取得 Snapshot，若 > 500 就跳過
                snaps = self.api.snapshots([stk_contract])
                if snaps:
                    ref_px = float(getattr(snaps[0], "close", 0) or getattr(snaps[0], "reference_price", 0) or 0)
                    if ref_px > 500:
                        # print(f"  [Filter] Skip high price stock {stock_code}: {ref_px}")
                        continue
            except Exception:
                pass # 查不到價格就放行，或保守擋掉
            # ---------------------------
...
class StrategyEngine:
    def __init__(self, market_data: MarketData, pos_mgr: PositionManager):
...
class ArbitrageSystem:
    def __init__(self):
        ...
        self.strategy = StrategyEngine(self.market_data, self.pos_mgr)
        self.execution = ExecutionEngine(self.market_data, self.pos_mgr)

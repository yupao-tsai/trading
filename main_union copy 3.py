...
def analyze_candidates(ib: IB, candidates: List[Option], current_price: float, hv_1w: float, hv_6m: float, iv_rank_info: Tuple[float, float, float] = (0,0,0), earnings_date: Optional[datetime.date] = None) -> List[Dict]:
    """對候選合約請求報價並計算指標"""
    results = []
...
def main():
    ...
    try:
        ...
        ranked_results = analyze_candidates(ib, candidates, price, hv_1w, hv_6m, (iv_low, iv_high, iv_curr_hist), earnings_date)
        ...
        sub_header = f"{'':<10} | {'':<4} | {'':<8} | {'':<16} | {'>5%':<6} | {'':<8} | {'>1.0':<6} | {'>50':<4} | {'>1.0':<5} | {'High':<5} | {'':<6} | {'':<6} | {'<.30':<7} | {'>.2':<4} | {'High':<5} | {'High':<7}"
        ...
        for res in ranked_results[:top_n]:
            ...
            # Format new metrics
            ivr_str = f"{res['ivr']:.0f}"
            safe_str = f"{res['safety']:.1f}"
            td_str = f"{res['td_ratio']:.2f}"
            
            # Highlight Earnings Collision with Red Text if present? (Will be handled by row color generally, but maybe specific marker?)
            # if res['earn']: earn_str = "!YES" # removed from table

            # 組合物件 (Added back HV and Sharpe)
            row_str = f"{res['expiry']:<10} | {res['dte']:<4} | {res['strike']:<8.1f} | {price_field} | {res['annualized']*100:<6.1f} | {res['ideal_bid']:<8.2f} | {ratio:<6.2f} | {ivr_str:<4} | {safe_str:<5} | {td_str:<5} | {iv_str:<6} | {hv_str:<6} | {delta_str:>7} | {res['sharpe']:<4.2f} | {theta_pct_str:<5} | {res['kelly']*100:>6.1f}%"
            ...
            if is_good_deal:
                ...
                # If earnings collision, maybe add a warning indicator to the row string or change color?
                if res['earn']:
                     row_str += f" {COLOR_RED}(EARN){COLOR_RESET}" if row_color == COLOR_RESET else f" (EARN)"

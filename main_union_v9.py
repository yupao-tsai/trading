...
import numpy as np
import sys
# Ensure current dir is in path to find modules
sys.path.append(os.getcwd())
try:
    from stock_model import StockProbabilityModel
    from stock_data_loader import preprocess_dataframe
    MODEL_AVAILABLE = True
except ImportError:
    print("Warning: ML modules not found. Model features will be disabled.")
...
def load_ml_model():
    ...
    try:
        device = torch.device('cpu') # Inference on CPU is fine/safer for threaded IB apps
        ...
        if os.path.exists(model_path):
            ...
            print(f"[ML] Model file {model_path} not found.")
    except Exception as e:
        print(f"[ML] Failed to load model: {e}")

def get_stock_prediction(ib: IB, contract: Stock, current_price: float = -1.0) -> Tuple[Optional[np.ndarray], float]:
    """
    Returns: (probability_map, ref_ma5)
    probability_map: (60, 100) numpy array
    ref_ma5: float (the denominator used for normalization)
    """
    if GLOBAL_MODEL is None:
        return None, 1.0

    print(f"[ML] Fetching data for {contract.symbol}...")
    ...
    # Convert to DF
    df = util.df(bars)
    # IB cols: date, open, high, low, close, volume, average, barCount
    # Map to loader expectation: Open, High, Low, Close, Volume
    rename_map = {
    ...
    # Compute Reference MA5 (last row) using same logic as loader
    # Loader calculates rolling MA5 on the whole series.
    # We duplicate clean derivation here to ensure sync.
    df['MA5_Ref'] = df['Close'].rolling(window=5).mean()
    if len(df) < 5: return None, 1.0
    ref_ma5 = df['MA5_Ref'].iloc[-1]

    # Preprocess with override
    features = preprocess_dataframe(df, override_last_close=current_price) # (N, 12)
    if features is None or len(features) < 60:
        print(f"[ML] Insufficient data length: {len(features) if features is not None else 0}")
        return None, 1.0

    # Take last 60 steps
    input_seq = features[-60:] # (60, 12)
    input_tensor = torch.tensor(input_seq, dtype=torch.float32).unsqueeze(0) # (1, 60, 12)

    try:
        with torch.no_grad():
            logits = GLOBAL_MODEL(input_tensor)
            probs = torch.sigmoid(logits).squeeze(0).numpy() # (60, 100)
        return probs, ref_ma5
    except Exception as e:
        print(f"[ML] Inference failed: {e}")
        return None, 1.0

def get_prob_below(probs: np.ndarray, ref_ma5: float, strike: float, dte: int, min_r=0, max_r=5, bins=100) -> float:
    """
    Calculate Prob(Price < Strike) at dte.
    """
    if probs is None: 
        return -1.0 # Indicator for N/A
    
    # Clamp day index (0..59)
    day_idx = min(dte, 60) - 1
    if day_idx < 0: day_idx = 0
    
    target_ratio = strike / ref_ma5
    
    # If target ratio is outside our grid, handle edge cases
    if target_ratio <= min_r: return 0.0
    if target_ratio >= max_r: return 1.0 # Should be 1.0 technically if strike is way above max price? Yes, Prob(P < Strike) -> 1.
    
    # Bins
    bin_edges = np.linspace(min_r, max_r, bins + 1)
    # bin_ends[i] is the upper bound of bin i
    bin_ends = bin_edges[1:]
    
    # Sum prob of bins fully below target
    # This is a discrete approximation
    mask = bin_ends <= target_ratio
    prob = np.sum(probs[day_idx][mask])
    
    return float(prob)

def calculate_expected_loss(probs: np.ndarray, ref_ma5: float, strike: float, dte: int, min_r=0, max_r=5, bins=100) -> float:
    """
    Calculate Expected Loss (Risk) per share: E[max(0, Strike - S_T)]
    Expected Value for Sell Put = Premium - Expected Loss
    """
    ...
    # Prob distribution for the specific day
    day_probs = probs[day_idx] # (100,)
    
    # Bin centers as ratios
    bin_edges = np.linspace(min_r, max_r, bins + 1)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2.0
    
    # Implied prices at bin centers
    bin_prices = bin_centers * ref_ma5
    
    # Loss at each bin: max(0, Strike - Price)
    losses = np.maximum(0.0, strike - bin_prices)
    
    # Weighted sum
    exp_loss = np.sum(day_probs * losses)
    return float(exp_loss)
    ...
    for i in range(0, len(candidates), chunk_size):
        ...
        for opt, ticker in tickers:
            ...
            if ml_probs is not None:
                prob_below = get_prob_below(ml_probs, ref_ma5, float(opt.strike), dte)
                if prob_below >= 0:
                    win_prob_ml = 1.0 - prob_below
                    kelly_ml = calculate_kelly_fraction(annualized, RISK_FREE_RATE, win_prob_ml, iv, hv_1w, hv_6m)
                    
                    # Expected Value calc
                    expected_loss = calculate_expected_loss(ml_probs, ref_ma5, float(opt.strike), dte)
                    # EV = Premium - ExpLoss. (Bid is per share premium)
                    ev = bid - expected_loss
                    # EV Yield: Return on Risk (Strike or Margin?) Usually Return on Capital. 
                    # If Cash Secured Put: Capital = Strike. 
                    # If Margin: Capital = ~20% of Strike. 
                    # Let's use Return on Strike for conservatism (ROA), or maybe just EV raw.
                    # User asked for "5% profit rate". Maybe (EV / Strike)? 
                    if float(opt.strike) > 0:
                        ev_yield = ev / float(opt.strike)
            ...
            # Ideal bid / Ratio (沿用舊邏輯，便於表格顯示)
            target_kelly = 0.1
            conservative_vol = max(iv, hv_1w, hv_6m)
            req_ret = target_kelly * 2 * (conservative_vol ** 2) + RISK_FREE_RATE
            abs_delta = abs(delta)
            denom = (1.0 - abs_delta) if (1.0 - abs_delta) > 0.1 else 0.1
            req_ann = req_ret / denom
            ideal_bid = float(opt.strike) * req_ann * (dte / 365.0)
            ratio = (bid / ideal_bid) if ideal_bid > 0 else 0.0
...
def get_moomoo_positions(ib_symbol: str, underlying_price: float, hv_1w: float, hv_6m: float, iv_rank_info: Tuple[float, float, float]):
    ...
    try:
        ...
        print("\n--- Position Analysis (Calculated as if opening now) ---")
        print("Note: IV/Delta估算，Ann%/Kelly%為當前價假設新開倉計算")
        
        header = (
            f"{'Expiry':<10} | {'DTE':<4} | {'Strike':<8} | {'Price(Src)':<16} | {'Qty':<4} | {'P/L%':<7} | "
            f"{'Ann%':<6} | {'Ideal':<8} | {'Ratio':<6} | {'IVR':<5} | {'IV':<7} | {'HV':<7} | {'Delta':<8} | "
            f"{'Shp':<5} | {'T/D':<6} | {'Th%':<6} | {'Kelly%':<7}"
        )
        print(header)
        print("-" * len(header))
        ...
        for _, row in filtered_sorted.iterrows():
            code = row['code']
            m = opt_pattern.search(code)
            if not m:
                continue
            date_str, opt_type, strike_str = m.groups()
            ...
            if 'current_price' in row and row['current_price'] and not math.isnan(row['current_price']):
                current_opt_price = float(row['current_price'])
            elif 'nominal_price' in row and row['nominal_price'] and not math.isnan(row['nominal_price']):
                current_opt_price = float(row['nominal_price'])
            elif 'cost_price' in row and row['cost_price'] and not math.isnan(row['cost_price']):
                current_opt_price = float(row['cost_price'])

            fees_per_share = FEES_PER_CONTRACT / 100.0
            net_bid = max(0.0, current_opt_price - fees_per_share)
            ret = net_bid / strike if strike > 0 else 0.0
            ...
            delta = bs_call_delta(underlying_price, strike, T, RISK_FREE_RATE, iv) if opt_type == 'C' else bs_put_delta(underlying_price, strike, T, RISK_FREE_RATE, iv)
            theta_annual = bs_put_theta(underlying_price, strike, T, RISK_FREE_RATE, iv) if opt_type == 'P' else 0.0
            theta = theta_annual / 365.0 if theta_annual else 0.0

            target_kelly_allocation = 0.10
            conservative_vol = max(iv, hv_1w, hv_6m)
            required_exp_ret = target_kelly_allocation * 2 * (conservative_vol ** 2) + RISK_FREE_RATE
            abs_delta = abs(delta)
            denom = (1.0 - abs_delta) if (1.0 - abs_delta) > 0.1 else 0.1
            required_ann_ret = required_exp_ret / denom
            ideal_bid = strike * required_ann_ret * (dte / 365.0)
            ratio = current_opt_price / ideal_bid if ideal_bid > 0 else 0.0

            win_prob = max(0.0, 1.0 - abs_delta)
            ...
            pl_str = f"{pl_val:.1f}%" if pl_val is not None and not math.isnan(pl_val) else "n/a"
            qty = int(row['qty']) if 'qty' in row and not math.isnan(row['qty']) else 0

            # --- Extra Metrics for Unification ---
            theta_yield = abs(theta) / current_opt_price if current_opt_price > 0 and theta != 0 else 0.0
            td_ratio = abs(theta) / abs(delta) if abs(delta) > 0.001 else 0.0
            ...
            price_src = f"{current_opt_price:.2f}(M)"
            if len(price_src) < 16:
                 price_src = price_src + " " * (16 - len(price_src))

            iv_str = f"{iv:.2f}"
            delta_str = f"{delta:.2f}"
            ann_str = f"{annualized_ret*100:.1f}"
            th_str = f"{theta_yield*100:.1f}%"
            kelly_str = f"{kelly_pct*100:.1f}%"
            ideal_str = f"{ideal_bid:.2f}"
            ratio_str = f"{ratio:.2f}"
            td_str = f"{td_ratio:.2f}"
            ivr_str = f"{ivr:.0f}"
            hv_str = f"{hv_val:.2f}"

            row_str = (
                f"{expiry_display:<10} | {dte:<4} | {strike:<8.2f} | {price_src} | {qty:<4} | {pl_str:<7} | "
                f"{ann_str:<6} | {ideal_str:<8} | {ratio_str:<6} | {ivr_str:<5} | {iv_str:<7} | {hv_str:<7} | {delta_str:<8} | "
                f"{sharpe:<5.2f} | {td_str:<6} | {th_str:<6} | {kelly_str:<7}"
            )
            print(row_str)
...
def scan_one_symbol(ib: IB, symbol: str, top_n: int) -> Tuple[List[Dict], float, Optional[datetime.date]]:
    ...
    # Get ML Prediction (with live price override)
    ml_probs, ref_ma5 = get_stock_prediction(ib, stk, current_price=price)
    
    ranked = analyze_candidates(ib, symbol, candidates, price, hv_1w, hv_6m, (iv_low, iv_high, iv_curr_hist), earnings_date, ml_probs, ref_ma5)

    # Console table (top N)  <-- DO NOT CHANGE
    print(f"\n====== Sell Put Recommendations for {symbol} (Price: {price:.2f}) ======")
    print("Thresholds: Ann>=15%, IVR>30, T/D>0.4, |Delta|<0.30, IV>HV")
    ...
    header = (
        ...
        f"{'Ann%':<6} | {'Ideal':<8} | {'Ratio':<6} | {'IVR':<5} | {'IV':<7} | {'HV':<7} | {'Delta':<8} | "
        f"{'Shp':<5} | {'T/D':<6} | {'Th%':<6} | {'Kelly':<6} | {'K-ML':<6} | {'EV%':<6}"
    )
    sub_header = (
        f"{'':<10} | {'':<4} | {'':<8} | {'':<16} | "
        f"{'>15%':<6} | {'':<8} | {'>1.0':<6} | {'>30':<5} | {'>HV':<7} | {'':<7} | {'<0.30':<8} | "
        f"{'':<5} | {'>0.4':<6} | {'High':<6} | {'High':<6} | {'High':<6} | {'>0':<6}"
    )
    print(header)
    print(sub_header)
    print("-" * len(header))
    ...
    for r in ranked[:top_n]:
        ...
        kelly_ml_str = f"{r.get('kelly_ml', 0.0)*100:.1f}%"
        ev_str = f"{r.get('ev_yield', 0.0)*100:.2f}%"
        ideal_str = f"{r.get('ideal_bid', 0.0):.2f}"
        ratio_str = f"{r.get('ratio', 0.0):.2f}"
        td_str = f"{r['td_ratio']:.2f}"
        ...
        row = (
            ...
            f"{ann_str:<6} | {ideal_str:<8} | {ratio_str:<6} | {r['ivr']:<5.0f} | {iv_str:<7} | {hv_val:<7.2f} | {delta_str:<8} | "
            f"{r['sharpe']:<5.2f} | {td_str:<6} | {th_str:<6} | {kelly_str:<6} | {kelly_ml_str:<6} | {ev_str:<6}"
        )

...
def get_moomoo_positions(ib_symbol: str, underlying_price: float, hv_1w: float, hv_6m: float):
    ...
    try:
        ...
        for _, row in filtered_sorted.iterrows():
            ...
            print(
                f"{expiry_display:<10} | {dte:<4} | {strike:<8.2f} | {current_opt_price:<8.2f} | "
                f"{qty:<4} | {pl_str:<7} | {annualized_ret*100:<6.1f} | {ideal_bid:<8.2f} | {ratio:<6.2f} | "
                f"{sharpe:<4.2f} | {iv:<6.2f} | {delta:<7.2f} | {kelly_pct*100:<7.1f}"
            )
...
def scan_one_symbol(ib: IB, symbol: str, top_n: int) -> Tuple[List[Dict], float, Optional[datetime.date]]:
    ...
    if not chains:
        return [], hv_val, earnings_date, hv_1w, hv_6m, price
    ...
    return ranked, hv_val, earnings_date, hv_1w, hv_6m, price
...
def main():
    ...
    try:
        ...
        for idx, sym in enumerate(symbols, 1):
            ...
            try:
                ranked, hv_val, earnings_date, hv_1w, hv_6m, underlying_price = scan_one_symbol(ib, sym, args.top)
            except Exception as e:
            ...
            if args.symbol:
                get_moomoo_positions(sym, underlying_price, hv_1w, hv_6m)

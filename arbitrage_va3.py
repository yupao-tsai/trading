...
try:
    ...
    if not getattr(_core_module, "_patched_by_bot", False):
        ...
        print(f"[Patch] pysolace fully monkeypatched (Core Request + OnReply + TokenCheck).")
    else:
...
AUTO_CLOSE_BROKEN_LEGS = False # 啟動時是否自動平倉壞部位 (Danger!)
...
class Reconciler:
    ...
    def _match_unknown_futures(self, snap: AccountSnapshot):
        ...
        # 1. Try Strict Match (Underlying ID)
        try:
        ...
        if SIMULATION and not matches and len(unknown_stocks) == 1 and len(held_futures) == 1:
             s = unknown_stocks[0]
             f = held_futures[0]
             print(f"  [Reconciler] ⚠️ Force-linking {s} -> {f} (Fallback for Simulation Data)")
             matches[s] = f
...
class KeyboardMonitor(threading.Thread):
    ...
    def _print_help_once(self):
        print("\n[Keyboard] p=positions, o=orders, h=help, f=force_close_code, q=quit\n", flush=True)
    ...
    def _handle_key(self, ch: str):
        ...
        if ch == "p":
            ...
            # Simple interactive force close
            print("\n[Keyboard] Enter stock code to Force Close (or empty to cancel): ", end='', flush=True)
            try:
                # This input() is blocking but runs in a thread, so it's okay for main loops.
                # However, Python input() might conflict with some daemon setups. 
                # Assuming standard console usage.
                import sys
                line = sys.stdin.readline().strip()
                if line:
                    print(f"[Keyboard] Requesting force close for '{line}'")
                    self.system.exec_engine.force_close_position(self.system.api, line)
                else:

...
class PairState:
    ...
    # open/close targets
    target_stock_lots: int = 0  # ✅ 改為 lots (張)
    ...
    open_ts: float = 0.0
    close_ts: float = 0.0
    unhedged_ts: float = 0.0  # Record when it became unhedged

    # State Guards (P0)
    inflight: bool = False    # 是否有進行中的流程 (下單/取消中)
    last_close_ts: float = 0.0 # 上次平倉/結束時間 (for Cooldown)

    # EWMA State
    last_tick_ts: float = 0.0
    basis_mean: float = 0.0   # EWMA Mean of Basis
    basis_std: float = 0.0    # EWMA Std (or MAD) of Basis
    sample_count: int = 0     # 樣本數計數 (Warmup)
    initialized: bool = False
    sample_count: int = 0     # For Warmup

    # Inflight Guard (P0)
    inflight: bool = False
    last_entry_ts: float = 0.0
    samples_count: int = 0    # Sample count for warmup
...
class PositionManager:
    ...
    def set_inflight(self, stock_code: str, val: bool):
        # Helper used by ExecutionEngine to release lock manually
        if val:
            self.try_acquire_lock(stock_code)
        else:
            self.release_lock(stock_code)
    ...
    def set_inflight(self, stock_code: str, is_inflight: bool):
        with self._lock:
            ps = self._pairs.get(stock_code)
            if ps:
                ps.inflight = is_inflight
                if not is_inflight:
    ...
    def set_inflight(self, stock_code: str, inflight: bool):
        with self._lock:
            ps = self._pairs.get(stock_code)
            if ps:
                ps.inflight = inflight
                if not inflight:
                    ps.last_close_ts = time.time() # Start cooldown
    ...
    def monitor_sync(self, stock_code: str, pos_stock_shares: int, pos_fut_qty: int):
        ...
        with self._lock:
            ...
            # unless state is EMPTY or HOLDING (stable states).
            # If we are PENDING_ENTRY, we trust ExecutionEngine to finish.
            ...
            # 單邊 -> UNHEDGED（保留你原本邏輯）
                ...
                if ps.state not in (PositionState.PENDING_ENTRY, PositionState.PENDING_EXIT):
                    if ps.state != PositionState.UNHEDGED:
                        ps.unhedged_ts = time.time() # Start cool-down
                        print(f"[Monitor Sync] {stock_code} Detected UNHEDGED -> Trigger Repair Logic")
                    ps.state = PositionState.UNHEDGED
...
class StrategyEngine:
    ...
    def check_risk(self, is_open: bool = True) -> bool:
        ...
        if is_open:
            ...
            unhedged = 0
            for ps in self.pos_mgr.all_pairs():
                if ps.state in (PositionState.HOLDING, PositionState.PENDING_ENTRY, PositionState.PENDING_EXIT, PositionState.UNHEDGED):
                    active += 1
                if ps.state == PositionState.UNHEDGED:
                    unhedged += 1
    ...
    def on_quote(self, quote: Any) -> Optional[TradeSignal]:
        ...
        if ps.inflight:
            # P0: Timeout check for stuck inflight? 
            # If inflight for > 10s, something is wrong, release?
            # Let ExecutionEngine handle strict timeouts.
            return None
            
        if ps.last_close_ts > 0 and (now - ps.last_close_ts < INFLIGHT_COOLDOWN_SEC):
            return None
        # --------------------------------------
    ...
    def on_signal(self, sig: Dict[str, Any], api: sj.Shioaji):
        ...
        try:
            ...
            if (not filled_s) and (not filled_f):
                ...
                # 如果真的沒 Cancel 掉且成交了，會由 AccountMonitor 抓出 UNHEDGED
                if c1 and c2:
                    self.pos_mgr.set_state(stock_code, PositionState.EMPTY if is_open else PositionState.HOLDING)
                else:
                    # 如果連送出 Cancel 都失敗，標記為 UNHEDGED 比較安全
                    print(f"[{phase.upper()}] cancel failed -> set UNHEDGED")
                    self.pos_mgr.set_state(stock_code, PositionState.UNHEDGED)
                self.pos_mgr.set_inflight(stock_code, False) # Unlock
                return
            ...
            if is_open:
                ...
                if filled_f and (not filled_s):
                    ...
                    # ✅ UNWIND STRATEGY: Aggressive LMT
                    print(f"  [Unwind-to-HOLDING] sell future again -> Aggressive LMT")
                    try:
                        px = self._mk_mktable_price_strict("sell", f_bid, protect_ticks=10)
                        trade_fix = self._place_future_order(
                            api, future_code, sj.constant.Action.Sell, px, FUTURE_QTY, phase_open=True,
                ...
                if filled_s and (not filled_f):
                    ...
                    # ✅ UNWIND STRATEGY: Aggressive LMT
                    print(f"  [Unwind-to-HOLDING] buy stock back -> Aggressive LMT")
                    try:
                        px = self._mk_mktable_price_strict("buy", s_ask, protect_ticks=10)
                        trade_fix = self._place_stock_order(
                            api, stock_code, sj.constant.Action.Buy, px, STOCK_QTY,
            ...
            self.pos_mgr.set_inflight(stock_code, False) # Unlock Safety

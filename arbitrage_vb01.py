...
class BaseTransaction:
    ...
    def _step_place_leg2(self):
        ...
        if not fut_snap:
            ...
            self.set_state(TransactionState.FAILED, "Leg 2 missing future market data -> repaired")
            return
...
class CloseArbitrageTransaction(BaseTransaction):
    ...
    def _step_close_init_buy_future(self):
        ...
        if not self._book_ok_for_market(snap, "Buy", self.mgr.MIN_FUT_BOOK_QTY, "Future"):
            self.set_state(TransactionState.FAILED, "Close leg1 BLOCKED by Future book size")
            return
    ...
    def _step_close_leg2_sell_stock(self):
        ...
        if not snap or (snap.price <= 0 and snap.bid <= 0 and snap.ask <= 0):
            self.log("No Stock market data for close leg2. Auto-Repair.")
            self.mgr.repair_positions()
            self.set_state(TransactionState.FAILED, "Close leg2 missing stock market data -> repaired")
            return

        # Gate by book size for SELL stock
        if not self._book_ok_for_market(snap, "Sell", self.mgr.MIN_STK_BOOK_QTY, "Stock"):
            self.log("Close leg2 BLOCKED by Stock book size -> Auto-Repair")
            self.mgr.repair_positions()
            self.set_state(TransactionState.FAILED, "Close leg2 BLOCKED by Stock book size -> repaired")
            return
        ...
        if not seqno:
            self.log("Close leg2 submit failed -> Auto-Repair")
            self.mgr.repair_positions()
            self.set_state(TransactionState.FAILED, "Close leg2 (Stock Sell) submit failed -> repaired")
            return
    ...
    def _check_close_leg2_polling(self):
        ...
        if tgt > 0 and (s_str == "Filled" or qty >= tgt):
            ...
            self.log("Close leg2 cancelled -> Auto-Repair")
            self.mgr.repair_positions()
            self.set_state(TransactionState.FAILED, "Close leg2 cancelled -> repaired")
    ...
    def _check_close_leg2_timeout(self):
        if time.time() - self.state_enter_ts > 90:
            ...
            self.mgr.execution.cancel_order_by_seqno(self.stock_order.seqno)
            self.log("Close leg2 timeout -> Auto-Repair")
            self.mgr.repair_positions()
            self.set_state(TransactionState.FAILED, "Close leg2 timeout & cancelled -> repaired")
...
class TransactionManager:
    ...
    def dump_status(self) -> str:
        ...
        for code, tx in self.active_transactions.items():
            ...
            lines.append(f"    Leg1: {tx.stock_order.status} ({tx.stock_order.filled_qty}/{tx.intent.qty}) seq={tx.stock_order.seqno}")
            lines.append(f"    Leg2: {tx.future_order.status} ({tx.future_order.filled_qty}) seq={tx.future_order.seqno}")

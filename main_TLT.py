...
SYMBOL = "TLT"
PRIMARY_EXCHANGE = "SMART" # TLT 主要是 ETF，用 SMART 即可
CURRENCY = "USD"

# 篩選條件
MIN_DTE = 7           # 最小到期天數
MAX_DTE = 60           # 最大到期天數
STRIKE_PCT_MIN = 1.02  # 履約價下限 (現價 * 1.02) - Sell Call 應 > 現價
STRIKE_PCT_MAX = 1.20  # 履約價上限 (現價 * 1.10)
...
def filter_options(
    ...
    # 2. 篩選履約價 (Call: Strike > Current Price)
    # 計算目標履約價範圍
    ...
    for i in range(0, len(candidates), chunk_size):
        ...
        for opt, ticker in tickers:
            ...
            if ticker.modelGreeks and ticker.modelGreeks.impliedVol:
                 ...
                 # 使用 Black-Scholes 反推 IV 與 Delta (Call)
                 iv = implied_volatility(bid, current_price, opt.strike, T, RISK_FREE_RATE)
                 delta = bs_call_delta(current_price, opt.strike, T, RISK_FREE_RATE, iv)
            ...
            # 而 Exp.Ret = Annualized_Ret * (1 - |Delta|) (Call 的勝率也是 1 - Delta，假設 OTM)
            ...
            abs_delta = abs(delta) # Call Delta 為正，取 abs 沒差
            
            # 加上 Delta 修正 (還原) - 對於 Sell OTM Call，勝率 = 1 - Delta
...
def main():
    ...
    try:
        ...
        # 5. 輸出結果
        print(f"\n====== Sell Call Recommendations for {SYMBOL} (Price: {price}) ======")

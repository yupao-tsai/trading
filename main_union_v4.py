...
def analyze_candidates(
    ...
    for i in range(0, len(candidates), chunk_size):
        ...
        for opt, ticker in tickers:
            ...
            buffer_pct = (current_price - opt.strike) / current_price if current_price > 0 else 0.0
...
def format_candidate_line(r: Dict, hv_val: float) -> str:
    # Consistent, parse-friendly: TSV
    # columns: symbol, expiry, dte, strike, bid, src, ann%, ivr, iv, hv, delta, sharpe, td, safe, theta%
    ann = r["annualized"] * 100.0
    theta_pct = r["theta_yield"] * 100.0 if r.get("theta_yield") else 0.0
    return (
        f"{r['symbol']}\t{r['expiry']}\t{r['dte']}\t{r['strike']:.2f}\t"
        f"{r['bid']:.2f}\t{r['price_src']}\t{ann:.2f}\t{r['ivr']:.2f}\t"
        f"{r['iv']:.4f}\t{hv_val:.4f}\t{r['delta']:.4f}\t{r['sharpe']:.4f}\t"
        f"{r['td_ratio']:.4f}\t{r['safety']:.4f}\t{theta_pct:.2f}"
    )
...
def is_perfect_candidate(r: Dict, hv_val: float) -> bool:
    ...
    # safe is informative; you can uncomment to hard enforce:
    # safe_ok = r["safety"] > PERFECT_MIN_SAFE
...
def main():
    ...
    # Default overwrite (clean run). Use --append if you want incremental.
    TSV_HEADER = "\t".join([
        "Symbol", "Expiry", "DTE", "Strike", "Bid(Src)",
        "Ret%", "Ann%", "Ideal", "Ratio", "Shp",
        "IV", "HV", "Theta", "Delta", "Kelly%", "IVR", "T/D"
    ])
    if not args.append:
            ...
            with open(out_file, "a", encoding="utf-8") as f:
                for r in ranked:
                    if is_perfect_candidate(r, hv_val):
                        ...
                        # include earnings collision flag at end for quick check

...
def main():
    ...
    try:
        ...
        # Header with thresholds - Updated based on advice
        # IVR > 30, Delta < 0.30, Ann% > 15%, IV > HV, T/D > 0.4
        ...
        for res in ranked_results[:top_n]:
            ...
            def color_cell(text, condition):
                if is_perfect: return text # Row will be all red
            ...
            # 判斷是否為優質交易 (Legacy check, primarily for Ratio column logic or specific highlighting if needed)
            ...
            # Apply Color to Cells (if not perfect row)
            # Ann%
            ...
            # Other fields (Legacy logic: if is_good_deal maybe color Ratio?)
            # For now, only color the specific metrics requested.
            
            # Construct Row
            # Note: Formatting widths must match header
            row_str = (

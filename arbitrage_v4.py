...
STOCK_QTY = 2          # 現貨口數 (張)
FUTURE_QTY = 1         # 期貨口數 (口)
ENTRY_THRESHOLD = 500 # 進場價差門檻 (金額)
EXIT_THRESHOLD = 0     # 出場價差門檻 (金額)

MIN_VOLUME_THRESHOLD = 500  # 自動篩選：期貨日均量門檻 (口) [模擬環境調整]
VOLUME_HISTORY_FILE = "volume_history.json"  # 交易量記錄檔

# ✅ 新增：動態換監控組合參數
TOP_N_PAIRS = 30               # 程式一開始抓成交量最大的 N 組
REFRESH_PAIRS_EVERY_SEC = 60000   # 每隔幾秒重找一次更適合的 pair（可調）
MAX_SUBS = 80                  # 訂閱上限（保守值；若你確認可更大自行改）
...
DBG_WATCH_FUT: Optional[str] = None  # 會在 start() 後自動填入
DBG_LAST_LINE = None
...
class KeyboardMonitor(threading.Thread):
    ...
    def _handle_key(self, ch: str):
        ...
        if not ch:
            return
        if ch == "p":
            self.system.print_positions()
        elif ch == "o":
            self.system.print_orders()
        elif ch == "h":
            self._print_help_once()
        elif ch == "q":
...
class WatchPrinter:
    def __init__(self, stk_code: str, fut_code: str, hedge_ratio: int = 2000):
        self.stk = stk_code
        self.fut = fut_code
        self.hedge_ratio = hedge_ratio

        self._lock = threading.Lock()
        self._stk_bid = self._stk_ask = None
        self._fut_bid = self._fut_ask = None
        self._last_tuple = None
        self._last_print_ts = 0.0

    def update_stk_bidask(self, bid, ask):
        with self._lock:
            self._stk_bid, self._stk_ask = bid, ask
        self._maybe_print()

    def update_fut_bidask(self, bid, ask):
        with self._lock:
            self._fut_bid, self._fut_ask = bid, ask
        self._maybe_print()

    def _maybe_print(self):
        # 可選：節流 20~50ms，避免一秒刷上百次把 terminal 卡死
        now = time.monotonic()
        if now - self._last_print_ts < 0.03:
            return

        with self._lock:
            cur = (self._stk_bid, self._stk_ask, self._fut_bid, self._fut_ask)
            if None in cur:
                return
            if cur == self._last_tuple:
                return
            self._last_tuple = cur
            self._last_print_ts = now

            stk_bid, stk_ask, fut_bid, fut_ask = cur
            stk_mid = (stk_bid + stk_ask) / 2
            fut_mid = (fut_bid + fut_ask) / 2
            net = int((fut_mid - stk_mid) * self.hedge_ratio)

            line = (f"[WATCH] {self.stk}/{self.fut} "
                    f"STK(b/a)={stk_bid:.2f}/{stk_ask:.2f} "
                    f"FUT(b/a)={fut_bid:.2f}/{fut_ask:.2f} "
                    f"net={net:+d}")

        # 單行覆寫（清行 + 回到行首）
        sys.stdout.write("\r" + " " * 160 + "\r" + line)
        sys.stdout.flush()
# watch = WatchPrinter("2313", "FTFA6", hedge_ratio=2000)
# =========================
# --- ArbitrageSystem ---
# =========================
...
class ArbitrageSystem:
    def __init__(self):
        ...
        self._dbg_quote_cnt = 0
        self._dbg_last_quote_ts = 0.0
        self._dbg_last_quote_code = None
    ...
    def _setup_callbacks(self):
        ...
        def on_stock_quote(self_api, exchange, quote):
            ...
            # self._dbg_quote_cnt += 1
            # self._dbg_last_quote_ts = time.time()
            # self._dbg_last_quote_code = quote.code
            # if self._dbg_quote_cnt % 200 == 0:
            #     print(f"[DBG] quotes={self._dbg_quote_cnt} last={self._dbg_last_quote_code} dt={time.time()-self._dbg_last_quote_ts:.3f}s")

            # ===== DBG WATCH (stock leg) =====
            # print(f"[DBG] stock quote: {quote.code}")
            global DBG_WATCH_FUT, DBG_WATCH_STOCK
            if quote.code == DBG_WATCH_STOCK:
        ...
        def on_future_quote(self_api, exchange, quote):
            ...
            # ===== DBG WATCH (future leg) =====
            global DBG_WATCH_FUT, DBG_WATCH_STOCK
            if quote.code == DBG_WATCH_FUT:
                self._dbg_print_watch_line()
    ...
    def _dbg_print_watch_line(self):
        """
        只要 2303 或其對應期貨任一腿價格變動，就用同一行更新，不刷屏。
        """
        global DBG_LAST_LINE
        global DBG_WATCH_FUT, DBG_WATCH_STOCK

        if not DBG_WATCH_FUT:
            return

        s_quote, f_quote = self.market_data.get_quotes(DBG_WATCH_STOCK, DBG_WATCH_FUT)
        if not s_quote or not f_quote:
            return

        s_ask = float(s_quote.ask_price[0]) if s_quote.ask_price else 0.0
        s_bid = float(s_quote.bid_price[0]) if s_quote.bid_price else 0.0
        f_ask = float(f_quote.ask_price[0]) if f_quote.ask_price else 0.0
        f_bid = float(f_quote.bid_price[0]) if f_quote.bid_price else 0.0

        # 只要任一價格變動才印
        line_key = (s_ask, s_bid, f_ask, f_bid)
        if DBG_LAST_LINE == line_key:
            return
        DBG_LAST_LINE = line_key
        ...
        cost = self.strategy_engine.calculate_cost(s_ask, f_bid) if self.strategy_engine else 0.0
        net = spread_value - cost

        msg = (
            f"[WATCH] {DBG_WATCH_STOCK}/{DBG_WATCH_FUT} "
            f"STK(bid/ask)={s_bid:.2f}/{s_ask:.2f} "
            f"FUT(bid/ask)={f_bid:.2f}/{f_ask:.2f} "
            f"net={net:,.0f}      "
        )
    ...
    def start(self, top_n_pairs: int = TOP_N_PAIRS, refresh_interval_sec: int = REFRESH_PAIRS_EVERY_SEC):
        ...
        # ====== DBG: 找 2303 對應期貨（只抓第一個匹配）======
        global DBG_WATCH_FUT, DBG_WATCH_STOCK
        # DBG_WATCH_FUT = None
        for s, f in self.active_pairs:
            if s == DBG_WATCH_STOCK:
                DBG_WATCH_FUT = f
                break
        print(f"[DBG] watch pair: {DBG_WATCH_STOCK}/{DBG_WATCH_FUT}")
